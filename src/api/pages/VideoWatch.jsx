...
import { ThumbsUp, MessageSquare, Share2, Loader2, Heart, ChevronRight } from "lucide-react";
...
export default function VideoWatch() {
  ...
  const realtimeChannel = id ? `video:${id}` : null; // optional
    ...
    <div className="min-h-screen bg-gradient-to-b from-black via-neutral-900 to-black text-white flex flex-col md:flex-row">
      ...
      <div className="flex-1 md:p-6">
        <div className="rounded-xl overflow-hidden border border-white/10 bg-black/60 backdrop-blur-md shadow-xl">
          <video src={video.videoUrl} controls autoPlay className="w-full aspect-video object-cover" />
        </div>
        ...
        <div className="mt-4 flex flex-col gap-3">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 text-sm text-white/60">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={`flex items-center gap-1 ${liked ? "text-amber-300" : "text-white/60 hover:text-amber-300"}`}
              >
                <ThumbsUp size={16} /> {liked ? "Liked" : "Like"}
              </button>
              <button
                onClick={() => setSupport(!support)}
                className={`flex items-center gap-1 ${support ? "text-pink-400" : "text-white/60 hover:text-pink-400"}`}
              >
          ...
          <div className="mt-3 flex items-center justify-between border-t border-white/10 pt-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/60">{video.creator?.followers || 0} followers</p>
              </div>
            </div>
            <a
          ...
          {/* Comments (real-time optional) */}
          <div className="mt-6">
            <Comments targetId={video._id} type="video" user={user} realtimeChannel={realtimeChannel} />
          </div>
        </div>
      </div>
