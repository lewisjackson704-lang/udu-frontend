...
import { Flame, Radio, RefreshCcw, PlayCircle } from "lucide-react";
...
export default function Home() {
  ...
  const [word, setWord] = useState({ term: "Create", meaning: "To make something unique — every day." });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black via-neutral-900 to-black text-white">
      ...
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-white/10 bg-black/50 px-4 py-3 backdrop-blur-md">
        <h1 className="text-xl font-bold text-amber-400">U·DU</h1>
        <button
          onClick={loadData}
      ...
      <main className="p-4 sm:p-6 space-y-10">
        {/* Word of the Day */}
        <section className="rounded-2xl border border-amber-400/30 bg-black/60 p-6 text-center shadow-md backdrop-blur-md">
          <h2 className="text-amber-400 font-bold text-lg mb-2">Word of the Day</h2>
          <p className="text-3xl font-extrabold text-amber-300 tracking-wide">{word.term}</p>
              ...
              {live.slice(0, 8).map((stream) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end p-3 bg-gradient-to-t from-black/70 via-black/20 to-transparent">
                    ...
                    <p className="text-xs text-white/60">{stream.creatorName || "Live Creator"}</p>
                  </div>
